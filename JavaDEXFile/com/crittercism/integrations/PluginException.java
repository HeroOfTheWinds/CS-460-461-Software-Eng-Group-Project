...
public class PluginException extends Throwable {
    ...
    public static PluginException createUnityException(String str, String str2) {
        try {
            return unsafeCreateUnityException(str, str2);
        } catch (ThreadDeath e) {
            throw e;
        } catch (Throwable th) {
            dx.m776a("Unable to log unity exception <" + str + "> " + str2, th);
            return null;
        }
    }
    ...
    private static PluginException unsafeCreateUnityException(String str, String str2) {
        ...
        for (indexOf = 0; indexOf < split.length; indexOf++) {
            ...
            if (trim3.length() != 0) {
                ...
                if (lastIndexOf == -1) {
                    dx.m779b("Unable to parse unity exception.  No class and method found for frame frame <" + trim3 + ">" + trim2);
                    return null;
                } else if (lastIndexOf == str3.length() - 1) {
                    dx.m779b("Unable to parse unity exception.  Method is zero length for frame <" + trim3 + ">" + trim2);
                    return null;
                } else {
                    ...
                    if (indexOf2 != -1) {
                        ...
                        if (split2.length >= 2) {
                            ...
                            try {
                                ...
                                trim = trim3.substring(0, (trim3.length() - str4.length()) - 1);
                    ...
                    stackTraceElementArr[indexOf] = new StackTraceElement(substring, substring2, trim, i);
                }
            }
        }
