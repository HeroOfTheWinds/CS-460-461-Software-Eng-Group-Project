...
public class DiskBasedCache implements Cache {
        ...
        if (this.mTotalSize + ((long) i) >= ((long) this.mMaxCacheSizeInBytes)) {
            ...
            while (it.hasNext()) {
                ...
                if (getFileForKey(cacheHeader.key).delete()) {
                    ...
                    VolleyLog.m42d("Could not delete cache entry for key=%s, filename=%s", cacheHeader.key, getFilenameForKey(cacheHeader.key));
                }
                it.remove();
                i2 = i3 + 1;
            ...
            i2 = i3;
            if (VolleyLog.DEBUG) {
                VolleyLog.m45v("pruned %d files, %d bytes, %d ms", Integer.valueOf(i2), Long.valueOf(this.mTotalSize - j), Long.valueOf(SystemClock.elapsedRealtime() - elapsedRealtime));
            }
        }
    }
            ...
            if (cacheHeader == null) {
                ...
                try {
                    ...
                    try {
                        ...
                        e = e3;
                        try {
                            VolleyLog.m42d("%s: %s", fileForKey.getAbsolutePath(), e.toString());
                            remove(str);
                            if (countingInputStream == null) {
                                try {
                                    countingInputStream.close();
    ...
    L_0x0013:
        ...
        com.mopub.volley.VolleyLog.m43e(r0, r1);	 Catch:{ all -> 0x006b }
    L_0x0024:
        monitor-exit(r10);
        return;
    L_0x0026:
    ...
    L_0x0031:
        ...
        r6 = com.mopub.volley.toolbox.DiskBasedCache.CacheHeader.readHeader(r2);	 Catch:{ IOException -> 0x0072, all -> 0x0074 }
        ...
        synchronized (this) {
            ...
            try {
                ...
                if (cacheHeader.writeHeader(fileOutputStream)) {
                    ...
                    fileOutputStream.close();
                    VolleyLog.m42d("Failed to write header for %s", fileForKey.getAbsolutePath());
                    throw new IOException();
                }
            } catch (IOException e) {
                if (!fileForKey.delete()) {
                    VolleyLog.m42d("Could not clean up file %s", fileForKey.getAbsolutePath());
                }
            }
        }
    }
    ...
    public void remove(String str) {
        synchronized (this) {
            ...
            removeEntry(str);
            if (!delete) {
                VolleyLog.m42d("Could not delete cache entry for key=%s, filename=%s", str, getFilenameForKey(str));
            }
        }
    }
}
