...
public final class MathUtil {
    ...
    public static float ease(float f) {
        return (float) Math.sin((((double) Math.max(Math.min(DefaultRetryPolicy.DEFAULT_BACKOFF_MULT, f), 0.0f)) * 3.141592653589793d) / 2.0d);
    }
    ...
    public static float[] quadraticBezier(float f, float f2, float f3, float f4, float f5, float f6, int i) {
        ...
        for (int i2 = 0; i2 < i; i2++) {
            float f7 = ((float) i2) * (DefaultRetryPolicy.DEFAULT_BACKOFF_MULT / ((float) (i - 1)));
            fArr[i2 * 2] = ((((DefaultRetryPolicy.DEFAULT_BACKOFF_MULT - f7) * (DefaultRetryPolicy.DEFAULT_BACKOFF_MULT - f7)) * f) + ((((DefaultRetryPolicy.DEFAULT_BACKOFF_MULT - f7) * Default.HTTP_REQUEST_BACKOFF_MULT) * f7) * f3)) + ((f7 * f7) * f5);
            fArr[(i2 * 2) + 1] = ((f7 * f7) * f6) + ((((DefaultRetryPolicy.DEFAULT_BACKOFF_MULT - f7) * (DefaultRetryPolicy.DEFAULT_BACKOFF_MULT - f7)) * f2) + ((((DefaultRetryPolicy.DEFAULT_BACKOFF_MULT - f7) * Default.HTTP_REQUEST_BACKOFF_MULT) * f7) * f4));
        }
        return fArr;
    }
