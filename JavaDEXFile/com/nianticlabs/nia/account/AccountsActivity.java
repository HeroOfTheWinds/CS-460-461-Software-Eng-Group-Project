...
public class AccountsActivity extends Activity {
    ...
    class C07451 extends AsyncTask<Void, Void, Void> {
    ...
    class C07462 implements Runnable {
        ...
        C07462(UserRecoverableAuthException userRecoverableAuthException) {
    ...
    private void askUserToRecover(UserRecoverableAuthException userRecoverableAuthException) {
        runOnUiThread(new C07462(userRecoverableAuthException));
    }
    ...
    private void getAuth(String str) {
        new C07451(str).execute(new Void[0]);
    }
    ...
    private static void getAuthTokenBlocking(AccountsActivity accountsActivity, String str) {
        try {
            ...
            accountsActivity.accountManager.setAuthToken(Status.OK, GoogleAuthUtil.getToken((Context) accountsActivity, str, stringExtra), str);
            accountsActivity.postFinish();
        } catch (UserRecoverableAuthException e) {
            accountsActivity.askUserToRecover(e);
        } catch (Throwable e2) {
            Log.e(TAG, "Unable to get authToken at this time.", e2);
            accountsActivity.accountManager.setAuthToken(Status.NON_RECOVERABLE_ERROR, BuildConfig.FLAVOR, str);
            accountsActivity.postFinish();
        } catch (Throwable e22) {
