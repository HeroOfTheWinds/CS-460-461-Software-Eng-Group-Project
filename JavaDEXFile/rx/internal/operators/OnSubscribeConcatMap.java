...
public final class OnSubscribeConcatMap<T, R> implements OnSubscribe<R> {
    ...
    class C12831 implements Producer {
        final /* synthetic */ ConcatMapSubscriber val$parent;

        C12831(ConcatMapSubscriber concatMapSubscriber) {
        ...
        void drain() {
            if (this.wip.getAndIncrement() == 0) {
                ...
                while (!this.actual.isUnsubscribed()) {
                    if (!this.active) {
                        Throwable terminate;
                        if (i != OnSubscribeConcatMap.BOUNDARY || this.error.get() == null) {
                            boolean z = this.done;
    ...
    public void call(Subscriber<? super R> subscriber) {
        ...
        Subscriber concatMapSubscriber = new ConcatMapSubscriber(serializedSubscriber, this.mapper, this.prefetch, this.delayErrorMode);
        subscriber.add(concatMapSubscriber);
        subscriber.add(concatMapSubscriber.inner);
        subscriber.setProducer(new C12831(concatMapSubscriber));
