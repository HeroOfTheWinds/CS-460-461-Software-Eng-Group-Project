...
public final class OnSubscribeRefCount<T> implements OnSubscribe<T> {
    ...
    class C12981 implements Action1<Subscription> {
        final /* synthetic */ Subscriber val$subscriber;
        final /* synthetic */ AtomicBoolean val$writeLocked;

        C12981(Subscriber subscriber, AtomicBoolean atomicBoolean) {
    ...
    class C12992 extends Subscriber<T> {
        final /* synthetic */ CompositeSubscription val$currentBase;
        final /* synthetic */ Subscriber val$subscriber;

        C12992(Subscriber subscriber, Subscriber subscriber2, CompositeSubscription compositeSubscription) {
    ...
    class C13003 implements Action0 {
        final /* synthetic */ CompositeSubscription val$current;

        C13003(CompositeSubscription compositeSubscription) {
    ...
    private Subscription disconnect(CompositeSubscription compositeSubscription) {
        return Subscriptions.create(new C13003(compositeSubscription));
    }

    private Action1<Subscription> onSubscribe(Subscriber<? super T> subscriber, AtomicBoolean atomicBoolean) {
        return new C12981(subscriber, atomicBoolean);
    }
    ...
    void doSubscribe(Subscriber<? super T> subscriber, CompositeSubscription compositeSubscription) {
        ...
        this.source.unsafeSubscribe(new C12992(subscriber, subscriber, compositeSubscription));
    }
}
