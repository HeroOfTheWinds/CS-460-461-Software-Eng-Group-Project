...
public final class ScheduledAction extends AtomicReference<Thread> implements Runnable, Subscription {
    ...
    private final class FutureCompleter implements Subscription {
        ...
        public boolean isUnsubscribed() {
            return this.f912f.isCancelled();
        }

        public void unsubscribe() {
            if (ScheduledAction.this.get() != Thread.currentThread()) {
                this.f912f.cancel(true);
            } else {
                this.f912f.cancel(false);
            }
        }
    }
    ...
    private static final class Remover2 extends AtomicBoolean implements Subscription {
        ...
        final ScheduledAction f913s;

        public Remover2(ScheduledAction scheduledAction, SubscriptionList subscriptionList) {
            this.f913s = scheduledAction;
            this.parent = subscriptionList;
        }

        public boolean isUnsubscribed() {
            return this.f913s.isUnsubscribed();
        }

        public void unsubscribe() {
            if (compareAndSet(false, true)) {
                this.parent.remove(this.f913s);
            }
        }
    }
