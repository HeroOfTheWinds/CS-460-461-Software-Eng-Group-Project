...
public class ViewPager extends ViewGroup {
    ...
    private void calculatePageOffsets(ItemInfo itemInfo, int i, ItemInfo itemInfo2) {
        ...
        for (i2 = i + SCROLL_STATE_DRAGGING; i2 < size; i2 += SCROLL_STATE_DRAGGING) {
            ...
            if (itemInfo4.position == count + INVALID_POINTER) {
                this.mLastOffset = (itemInfo4.widthFactor + f4) - DefaultRetryPolicy.DEFAULT_BACKOFF_MULT;
            }
            itemInfo4.offset = f4;
    ...
    void initViewPager() {
        ...
        this.mCloseEnough = (int) (Default.HTTP_REQUEST_BACKOFF_MULT * f);
    ...
    void smoothScrollTo(int i, int i2, int i3) {
        ...
        float f = (float) i6;
        float f2 = (float) i6;
        float distanceInfluenceForSnapDuration = distanceInfluenceForSnapDuration(Math.min(DefaultRetryPolicy.DEFAULT_BACKOFF_MULT, (((float) Math.abs(i4)) * DefaultRetryPolicy.DEFAULT_BACKOFF_MULT) / ((float) clientWidth)));
        int abs = Math.abs(i3);
        if (abs > 0) {
