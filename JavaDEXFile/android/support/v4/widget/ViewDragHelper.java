...
public class ViewDragHelper {
    ...
    private int computeAxisDuration(int i, int i2, int i3) {
        ...
        float f = (float) i4;
        float f2 = (float) i4;
        float distanceInfluenceForSnapDuration = distanceInfluenceForSnapDuration(Math.min(DefaultRetryPolicy.DEFAULT_BACKOFF_MULT, ((float) Math.abs(i)) / ((float) width)));
        int abs = Math.abs(i2);
        return Math.min(abs > 0 ? Math.round(Math.abs(((distanceInfluenceForSnapDuration * f2) + f) / ((float) abs)) * 1000.0f) * EDGE_TOP : (int) (((((float) Math.abs(i)) / ((float) i3)) + DefaultRetryPolicy.DEFAULT_BACKOFF_MULT) * 256.0f), MAX_SETTLE_DURATION);
    }
