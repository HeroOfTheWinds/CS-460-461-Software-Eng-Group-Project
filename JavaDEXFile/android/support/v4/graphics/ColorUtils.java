...
public class ColorUtils {
    ...
    public static int HSLToColor(float[] fArr) {
        ...
        float f2 = fArr[1];
        float f3 = fArr[2];
        float abs = (DefaultRetryPolicy.DEFAULT_BACKOFF_MULT - Math.abs((Default.HTTP_REQUEST_BACKOFF_MULT * f3) - DefaultRetryPolicy.DEFAULT_BACKOFF_MULT)) * f2;
        f3 -= 0.5f * abs;
        float abs2 = abs * (DefaultRetryPolicy.DEFAULT_BACKOFF_MULT - Math.abs(((f / 60.0f) % Default.HTTP_REQUEST_BACKOFF_MULT) - DefaultRetryPolicy.DEFAULT_BACKOFF_MULT));
        switch (((int) f) / 60) {
    ...
    public static void RGBToHSL(int i, int i2, int i3, float[] fArr) {
        ...
        float f4 = max - min;
        float f5 = (max + min) / Default.HTTP_REQUEST_BACKOFF_MULT;
        if (max == min) {
            f = 0.0f;
            f2 = 0.0f;
        } else {
            f = max == f ? ((f2 - f3) / f4) % 6.0f : max == f2 ? ((f3 - f) / f4) + Default.HTTP_REQUEST_BACKOFF_MULT : ((f - f2) / f4) + 4.0f;
            f2 = f4 / (DefaultRetryPolicy.DEFAULT_BACKOFF_MULT - Math.abs((Default.HTTP_REQUEST_BACKOFF_MULT * f5) - DefaultRetryPolicy.DEFAULT_BACKOFF_MULT));
        }
        f = (f * 60.0f) % 360.0f;
        if (f < 0.0f) {
            f += 360.0f;
